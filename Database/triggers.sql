...
AS
BEGIN
    DECLARE @username VARCHAR(50), @name VARCHAR(100),
	@email VARCHAR(100),@password VARCHAR(100),
	@dob DATE, @dateJoined DATETIME;

    SELECT @username = INSERTED.username, 
		@name = INSERTED.name, @dob = INSERTED.dob,
        @dateJoined = INSERTED.dateJoined,
		@email = INSERTED.email,
		@password = INSERTED.password
    FROM INSERTED;

    PRINT 'New user inserted: Username = ' + @username +
	', Name = ' + @name + 
	', Email = ' + @email + ', password = '+@password +
	', Date of Birth = ' + CONVERT(VARCHAR, @dob, 120) +
	', Date Joined = ' + CONVERT(VARCHAR, @dateJoined, 120);
END;
...
AS
BEGIN
    DECLARE @username VARCHAR(50), @oldEmail VARCHAR(100),
	@newEmail VARCHAR(100), @oldName VARCHAR(100),
	@newName VARCHAR(100), @oldDob DATE,
	@newDob DATE, @oldDateJoined DATETIME, @newDateJoined DATETIME;
    ...
    PRINT 'the record of ' + @username +' is updated'+ 
          ...
          ', Old Date of Birth = ' + CONVERT(VARCHAR, @oldDob, 120) +
		  ', New Date of Birth = ' + CONVERT(VARCHAR, @newDob, 120) +
          ', Old Date Joined = ' + CONVERT(VARCHAR, @oldDateJoined, 120) +
		  ', New Date Joined = ' + CONVERT(VARCHAR, @newDateJoined, 120);
END;
/*delete trigger*/
...
AS
BEGIN
    DECLARE @artistID INT, @name VARCHAR(100), @bio VARCHAR(MAX),
...
AS
BEGIN
    DECLARE @artistID INT, @oldName VARCHAR(100),
	@newName VARCHAR(100), @oldBio VARCHAR(MAX),
	@newBio VARCHAR(MAX), @oldProfilepiclink VARCHAR(255),
	@newProfilepiclink VARCHAR(255);

    SELECT @artistID = INSERTED.artistID,
        @newName = INSERTED.name, @oldName = DELETED.name,
        @newBio = INSERTED.bio, @oldBio = DELETED.bio,
        @newProfilepiclink = INSERTED.profilepiclink,
		@oldProfilepiclink = DELETED.profilepiclink
    FROM INSERTED
        INNER JOIN DELETED ON INSERTED.artistID = DELETED.artistID;

    PRINT 'Artist updated: ArtistID = ' + CAST(@artistID AS VARCHAR(10)) + 
          ', Old Name = ' + @oldName + ', New Name = ' + @newName +
          ', Old Bio = ' + ISNULL(@oldBio, 'NULL') +
		  ', New Bio = ' + ISNULL(@newBio, 'NULL') +
          ', Old Profile Pic Link = ' + ISNULL(@oldProfilepiclink, 'NULL') +
		  ', New Profile Pic Link = ' + ISNULL(@newProfilepiclink, 'NULL');
END;
...
AS
BEGIN
    DECLARE @songID INT, @title VARCHAR(100),
	@artistID INT, @albumID INT,
	@genreName VARCHAR(50),
	@duration TIME,
	@audiolink VARCHAR(255), @releaseDate DATETIME;
...
AS
BEGIN
    DECLARE @songID INT, @oldTitle VARCHAR(100), @newTitle VARCHAR(100),
	@oldArtistID INT, @newArtistID INT, @oldAlbumID INT,
	@newAlbumID INT, @oldGenreName VARCHAR(50), @newGenreName VARCHAR(50),
	@oldDuration TIME, @newDuration TIME, @oldAudiolink VARCHAR(255),
	@newAudiolink VARCHAR(255), @oldReleaseDate DATETIME, @newReleaseDate DATETIME;
    ...
    PRINT 'Song updated: SongID = ' + CAST(@songID AS VARCHAR(10)) + 
          ...
          ', Old ArtistID = ' + CAST(@oldArtistID AS VARCHAR(10)) +
		  ', New ArtistID = ' + CAST(@newArtistID AS VARCHAR(10)) +
          ', Old AlbumID = ' + ISNULL(CAST(@oldAlbumID AS VARCHAR(10)), 'NULL') +
		  ', New AlbumID = ' + ISNULL(CAST(@newAlbumID AS VARCHAR(10)), 'NULL') +
          ', Old Genre = ' + ISNULL(@oldGenreName, 'NULL') + ', New Genre = ' + ISNULL(@newGenreName, 'NULL') +
          ', Old Duration = ' + CONVERT(VARCHAR, @oldDuration, 120) + ', New Duration = ' 
		  + CONVERT(VARCHAR, @newDuration, 120) +
          ', Old Audio Link = ' + @oldAudiolink + ', New Audio Link = ' + @newAudiolink +
          ', Old Release Date = ' + CONVERT(VARCHAR, @oldReleaseDate, 120) +
		  ', New Release Date = ' + CONVERT(VARCHAR, @newReleaseDate, 120);
END;
